...
const _SWORD_SCENE: PackedScene = preload("res://weapon/sword.tscn")
const _FIRE_BALL_SCENE: PackedScene = preload("res://weapon/fire_ball.tscn")
...
func _on_remove_last_pressed() -> void:
	...
	# 先从玩家身上摘掉，get_children 立刻变化；否则 queue_free 在帧末才移除，刷新仍会列出该项
	p.remove_child(last)
	last.queue_free()
	_refresh_weapon_list()
